...
class OsSwitchButton extends React.Component {
    ...
    render() {
        return (
            ...
            <button className={this.props.currentOs ==='mac'?'SwitchButton':'SwitchButton switch'} onClick={this.handleSwitchOs}> {this.props.currentOs==='win'?(<i className="fab fa-windows"></i>):(<i className="fab fa-apple"></i>)} </button>
            </div>
        )
    }
}
